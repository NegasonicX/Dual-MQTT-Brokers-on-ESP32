...
/**
 * Brief:
 * This code demonstrates data publishing and subscribing on two MQTT Brokers at the same
 *
 * GPIO status:
...
//.............MQTT Declarations.....................
#define mqttbroker1 "<FIRST MQTT BROKER URL>"                        //++ First MQTT Broker
#define mqttbroker2 "<SECOND MQTT BROKER URL>"                    //++ Second MQTT Broker
...
esp_mqtt_client_handle_t client1;                                       //++ First Client for first MQTT BROKER
esp_mqtt_client_handle_t client2;                                       //++ Second Client for second MQTT BROKER
...
void MQTT_TASK1()                                                                           //++ Task to publish data on FIRST MQTT
{
    int count = 0;
    ...
    while(1)
    {
        ...
        if(mqtt_con_flag_1==1)                                                              //++ If FIRST MQTT IS AVAILABLE
        {
        sprintf(count_topic, "{\"COUNT\":\"%d\",\"device\":%s}", count, mac_json);          //++ Make the JSON and publish on FIRST MQTT
        topic_id = esp_mqtt_client_publish(client1, data_topic, count_topic, 0, 1, 0);
        ESP_LOGI("First MQTT", "sent publish successful, on topic %s", data_topic);
        }
...
void MQTT_TASK2()                                                                           //++ Task to publish data on SECOND MQTT
{
    int count = 0;
    char count_topic[100];
    ...
    while(1)
    {
        ...
        if(mqtt_con_flag_2==1)                                                              //++ If SECOND MQTT IS AVAILABLE
        {
        sprintf(count_topic, "{\"COUNT\":\"%d\",\"device\":%s}", count, mac_json);          //++ Make the JSON and publish on SECOND MQTT
        topic_id = esp_mqtt_client_publish(client2, data_topic, count_topic, 0, 1, 0);
        ESP_LOGI("Second MQTT", "sent publish successful, on topic %s", data_topic);
        }
...
static esp_err_t mqtt_event_handler_cb1(esp_mqtt_event_handle_t event)                          //++ FIRST MQTT HANDLER
{
    int msg_id;
    switch (event->event_id)
    {
        case MQTT_EVENT_CONNECTED:
            ...
            mqtt_con_flag_1 = 1;
            ESP_LOGI(TAG_MQTT, "MQTT_EVENT_CONNECTED : First");

            msg_id = esp_mqtt_client_publish(client1, data_topic, dev_speci, 0, 1, 0);
            ESP_LOGI(TAG_MQTT, "sent publish successful, on topic %s", data_topic);
            msg_id = esp_mqtt_client_subscribe(client1, cmd_topic, 1);
        ...
        case MQTT_EVENT_DISCONNECTED:
            ...
            mqtt_con_flag_1 = 0;
            ESP_LOGI(TAG_MQTT, "MQTT_EVENT_DISCONNECTED : First");           
            break;
        ...
        case MQTT_EVENT_DATA:
            ...
            ESP_LOGI(TAG_MQTT, "MQTT_EVENT_DATA : FIRST MQTT BROKER");
            ESP_LOGI(TAG_MQTT, "DATA RECEIVED = %.*s\r\n", event->data_len, event->data);
            ESP_LOGI(TAG_MQTT, "TOPIC USED = %.*s\r\n", event->topic_len, event->topic);
            break;
        case MQTT_EVENT_ERROR:

            ESP_LOGI(TAG_MQTT, "MQTT_EVENT_ERROR : First");
            break;
...
static esp_err_t mqtt_event_handler_cb2(esp_mqtt_event_handle_t event)                        //++ SECOND MQTT HANDLER
{
    int msg_id;
    switch (event->event_id)
    {
        case MQTT_EVENT_CONNECTED:
            ...
            mqtt_con_flag_2 = 1;
            ESP_LOGI(TAG_MQTT, "MQTT_EVENT_CONNECTED : Second");
            
            msg_id = esp_mqtt_client_publish(client2, data_topic, dev_speci, 0, 1, 0);
            ESP_LOGI(TAG_MQTT, "sent publish successful, on topic %s", data_topic);
            msg_id = esp_mqtt_client_subscribe(client2, cmd_topic, 1);
            ESP_LOGI(TAG_MQTT, "sent subscribe successful, msg_id=%d", msg_id);
            break;
        ...
        case MQTT_EVENT_DISCONNECTED:
            ...
            mqtt_con_flag_2 = 0;
            ESP_LOGI(TAG_MQTT, "MQTT_EVENT_DISCONNECTED : Second");                       
            break;
        ...
        case MQTT_EVENT_DATA:
            ...
            ESP_LOGI(TAG_MQTT, "MQTT_EVENT_DATA : SECOND MQTT BROKER");
            ESP_LOGI(TAG_MQTT, "DATA RECEIVED = %.*s\r\n", event->data_len, event->data);
            ESP_LOGI(TAG_MQTT, "TOPIC USED = %.*s\r\n", event->topic_len, event->topic);        
            break;
        ...
        case MQTT_EVENT_ERROR:
            ...
            ESP_LOGI(TAG_MQTT, "MQTT_EVENT_ERROR : Second");
            break;
...
static void mqtt_event_handler1(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)          //++ CREATE HANDLE
{
    ESP_LOGD(TAG_MQTT, "Event dispatched from event loop base=%s, event_id=%d", base, event_id);
    mqtt_event_handler_cb1(event_data);
}

static void mqtt_event_handler2(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)          //++ CREATE HANDLE
{
    ESP_LOGD(TAG_MQTT, "Event dispatched from event loop base=%s, event_id=%d", base, event_id);
...
{
    esp_mqtt_client_config_t mqtt_cfg1 = {                                                          //++ PASSING FIRST MQTT BROKER
        .uri = mqttbroker1,
        .client_id = "first_client",
    };
    printf("First MQTT: %s \n", mqtt_cfg1.uri);

    client1 = esp_mqtt_client_init(&mqtt_cfg1);
    esp_mqtt_client_register_event(client1, ESP_EVENT_ANY_ID, mqtt_event_handler1, client1);
    esp_mqtt_client_start(client1);

    esp_mqtt_client_config_t mqtt_cfg2 = {                                                         //++ PASSING SECOND MQTT BROKER
        .uri = mqttbroker2,     
        .client_id = "another_client",
    };
    printf("Second MQTT: %s \n", mqtt_cfg2.uri);

    client2 = esp_mqtt_client_init(&mqtt_cfg2);
    esp_mqtt_client_register_event(client2, ESP_EVENT_ANY_ID, mqtt_event_handler2, client2);
